...
static irqreturn_t carddetect_irq(int irq, void *data)
{
	...
	dev_err(mmc_dev(sdhost->mmc),"*** %s %d cd_gpio = %d, present = %d***\n", __func__,__LINE__, plat->cd_gpio, 
...
int cardstatus_check(struct sdhci_host *sdhost)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(sdhost);
	struct tegra_sdhci_host *tegra_host = pltfm_host->priv;
	struct platform_device *pdev = to_platform_device(mmc_dev(sdhost->mmc));
...
static int tegra_sdhci_suspend(struct sdhci_host *sdhci, pm_message_t state)
{
	...
	printk("%s %s enabled=%u\n", __func__,
		mmc_hostname(sdhci->mmc), tegra_host->clk_enabled);
...
static int __devinit sdhci_tegra_probe(struct platform_device *pdev)
{
	...
	if (!strcmp(dev_name(mmc_dev(host->mmc)), "sdhci-tegra.3")) {
		...
		if (!strcmp(dev_name(mmc_dev(host->mmc)), "sdhci-tegra.0"))  {
			external_sd=host;
			printk("%s - %d: name=%s,  cd_gpio=%d\n",__FILE__,__LINE__, dev_name(mmc_dev(external_sd->mmc)), plat->cd_gpio);
		}
	pltfm_host->clk = clk;

...
int tmp401_thermal_get_temp_i(struct tmp401_data *data, long *temp)
{
	//printk("tmp401_thermal_get_temp \n");
	return tmp401_get_temp(&data->client->dev, NULL, temp);
}
...
#ifdef CONFIG_TEGRA_SKIN_THROTTLE	
	is_suspended = true;
	//printk("%s(): %d: is_suspended = %d\n", __func__, __LINE__, is_suspended);
#endif	
	return err;
}

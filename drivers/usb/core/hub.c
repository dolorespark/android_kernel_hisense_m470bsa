 ...
 init2:
	...
	if (need_debounce_delay) {
		...
		if (type == HUB_INIT2) {
			...
			//msleep(delay);
			//pr_info("%s debounce delay sleep(%dmsec) marked\n", __func__, delay);
			//msleep(delay);			
		}
	}
 init3:
				...
				struct usb_device *udev, unsigned int delay)
{
	...
	for (i = 0; i < PORT_RESET_TRIES; i++) {
		...
		case 0:
			...
			//msleep(10 + 40);
			//pr_info("%s reduced delay by 40msec\n", __func__);
			msleep(10);			
...
retry_port_reset:
	...
	if (oldspeed != USB_SPEED_UNKNOWN && oldspeed != udev->speed) {
		/* WAR: find some special HS device (ICERA E450), the speed has
		 * been changed from HS to FS during reset-resume, and retry of
		 * usb port reset can fix the issues.
		 */
		if (retry++ < 4) {
			dev_dbg(&udev->dev, "device reset changed speed, retry %d time\n",
					retry);

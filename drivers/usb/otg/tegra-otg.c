...
//Unlock for AC Charger
static int tegra_otg_set_usbunlock(struct otg_transceiver *otg){
	tegra_otg_notify_event(otg, USB_EVENT_CHARGER);
	return 0;
}
...
#ifdef CONFIG_CHARGER_TPS8003X
	else if(to == OTG_STATE_A_SUSPEND && from == OTG_STATE_A_SUSPEND)
	{
#ifdef   CONFIG_BOARD_US9230
		if(tps8003x_charger_type() != NONE_CHARGER) {
#else
		if(tps8003x_charger_type() == AC_CHARGER) {
#endif
			tegra_vbus_detect_event(false);
			tegra_otg_notify_event(otg, USB_EVENT_NONE);
		}
	}
#endif
...
//	}
	if ((val & USB_ID_INT_STATUS) || (val & USB_VBUS_INT_STATUS)) {
			if ( (tegra->otg.state == OTG_STATE_B_PERIPHERAL)&&
					((val & USB_ID_STATUS) && (val & USB_VBUS_STATUS)))
					printk("*** %s %d  peripheral state***\n", __func__,__LINE__);
			else {
					printk("**** %s  %d *****\n",__func__,__LINE__);
					tegra->int_status = val;
					schedule_work (&tegra->work);
...
static int tegra_otg_probe(struct platform_device *pdev)
{
	...
	tegra->otg.set_usbunlock = tegra_otg_set_usbunlock;

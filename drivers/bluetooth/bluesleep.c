	...
	if (bluesleep_can_sleep()) {
		...
		if (tegra_uart_is_tx_empty(bsi->uport)) {
			...
			&& !test_bit(BT_ASLEEP, &flags)) {
              ...
              BT_DBG("bluesleep_sleep_work wake up as both BT_EXT_WAKE and BT_ASLEEP are not set");
					...
					unsigned long count, void *data)
{
	...
	BT_DBG("bluesleep_write_proc_lpm");
	...
	if (b == '0') {
		...
		if (!has_lpm_enabled) {
			...
			if (bsi->uport == NULL)
			{
				BT_DBG("bsi->uport is NULL ........................ error");
			}
			else
			{
				BT_DBG("bsi->uport is not NULL");
			}
					...
					unsigned long count, void *data)
{
	...
	BT_DBG("bluesleep_write_proc_btwrite");
					...
					unsigned long count, void *data)
{
	...
	BT_DBG("bluepower_write_proc_btwake");
					...
					unsigned long count, void *data)
{
	...
	BT_DBG("bluesleep_write_proc_proto");
...
static int __init bluesleep_init(void)
{
	...
	ent = create_proc_entry("btwake", 0622, sleep_dir);
	if (ent == NULL) {
	...
	ent = create_proc_entry("proto", 0622, sleep_dir);
	if (ent == NULL) {
	...
	/* read/write proc entries */
	ent = create_proc_entry("lpm", 0622, sleep_dir);
	if (ent == NULL) {
	...
	/* read/write proc entries */
	ent = create_proc_entry("btwrite", 0622, sleep_dir);
	if (ent == NULL) {

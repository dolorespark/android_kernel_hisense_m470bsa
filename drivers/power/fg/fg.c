...
#define DEBUG  //commit this line for release version
 ...
 */
static void fg_count_cycle(struct cell_state *cell)
{
	if (cell->delta_q < 0) {

		cell->cycle_q -= cell->delta_q;
 ...
 */
static bool fg_check_chg_complete(struct cell_state *cell)
{
	bool ret = false;
 ...
 * is being charged.
 */
static void fg_charging(struct cell_state *cell)
{
	...
	cell->charge_cycle_q += cell->delta_q;
 ...
 * is being discahrged.
 */
static void fg_discharging(struct cell_state *cell)
{
	...
	cell->discharge_cycle_q -= cell->delta_q;
	fg_count_cycle(cell);
...
static void print_flags(struct cell_state *cell, char *dbg)
{
	strcpy(dbg, "");
...
static bool fg_can_ocv(struct cell_state *cell)
{
	...
	if ((cell->soc >= cell->config->ocv->flat_zone_low)
		&& (cell->soc <= cell->config->ocv->flat_zone_high)) {
			dev_dbg(cell->dev, "NOC: Flat zone SOC\n"); 
			return false;
		}
 ...
 */
static void fg_cc(struct cell_state *cell)
{
...
int fg_calibrate(const struct cal_config *cal, int val)
{
	val = (val * cal->slope) / 0x8000;
	val += cal->offset;
	return val;
}
		...
		short cur, short temperature)
{
	...
	if (!cell->init2) {
		cell->last_correction = now;
		cell->init2 = true;
	}

	/* Filter knowingly wrong voltage readings */
	if (voltage < 2800)
	...
	voltage = fg_calibrate(cell->config->cal_volt, voltage);
	cur = fg_calibrate(cell->config->cal_cur, cur);
	temperature = fg_calibrate(cell->config->cal_temp, temperature);
	...
	/* Update temperature and passed charge*/
	cell->temperature = temperature;
	cell->delta_q = delta_q;
	...
	dev_dbg(cell->dev,
		...
		"N/O;%4d;%4d ; "
		"CS;%4d ; EL;%5d ; "
		"F/C;%d;%d;%d; %s ; CP;%d TEMP;%d\n",
		cell->delta_q, cell->nac, cell->soc, cell->voltage, cell->av_voltage,
		...
		cell->fcc, cell->cycle_count, cell->cycle_q, dbg, tmp,cell->temperature);
}

...
int alarm_set_rtc_alarm(struct timespec new_alarm_time)
{
    ...
    printk("(%d-%02d-%02d %02d:%02d:%02d UTC)\n",
            rtc_alarm.time.tm_year + 1900, rtc_alarm.time.tm_mon + 1, rtc_alarm.time.tm_mday,
...
int alarm_clear_rtc_alarm(void)
{
...
static int alarm_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (tmp_queue->first)
	{
		printk("[alarm]wakeup_queue include alarm[0]\n");
	...
	{
		printk("[alarm]wakeup_queue include alarm[2]\n");
		wakeup_queue = tmp_queue;
	}
#if defined(CONFIG_RTC_ALARM_POWEROFF_SUPPORT)  
        ...
        if (tmp_queue->first && (!wakeup_queue ||
                    ...
                    hrtimer_get_expires(&wakeup_queue->timer).tv64))
        {
                printk("[alarm]wakeup_queue include alarm[4]\n");
                wakeup_queue = tmp_queue;
        }
#endif
	if (wakeup_queue) {
...
#if defined(CONFIG_RTC_ALARM_POWEROFF_SUPPORT)
extern int tps80031_clear_alarm(void);
void alarm_shutdown(void)
{
    struct rtc_wkalrm   rtc_alarm;
    struct rtc_time     rtc_current_rtc_time;
    unsigned long       rtc_alarm_time;
    unsigned long       rtc_temp_time;
    struct timespec     rtc_current_timespec;
    struct timespec     rtc_delta;
    struct alarm_queue *wakeup_queue = NULL;
    struct alarm_queue *tmp_queue = NULL;
    ...
    printk("[alarm_shutdown]enter\n");

    tmp_queue = &alarms[ANDROID_ALARM_POWEROFF_WAKEUP];

    rtc_temp_time = timespec_sub(ktime_to_timespec(
        hrtimer_get_expires(&tmp_queue->timer)),
        rtc_delta).tv_sec;

    printk("[alarm_shutdown] rtc_temp_time = 0x%x\n", rtc_temp_time);
    if((!tmp_queue) || (rtc_temp_time == 0) )
    {
        wakeup_queue = NULL;
        printk("[alarm_shutdown] will clear all wakeup alarm.\n");
    }
    else
    {
		tmp_queue = &alarms[ANDROID_ALARM_POWEROFF_WAKEUP];
		printk("[alarm_shutdown]wakeup_queue include alarm[4]\n");
		wakeup_queue = tmp_queue;
    }
    ...
    if (wakeup_queue) {
        ...
        rtc_time_to_tm(rtc_alarm_time, &rtc_alarm.time);
        pr_alarm( ERROR, "alarm_shutdown, set rtc alarm type %d: (%d-%02d-%02d %02d:%02d:%02d)\n",
            ANDROID_ALARM_POWEROFF_WAKEUP,
            rtc_alarm.time.tm_year + 1900, rtc_alarm.time.tm_mon + 1, rtc_alarm.time.tm_mday,
            rtc_alarm.time.tm_hour+8, rtc_alarm.time.tm_min, rtc_alarm.time.tm_sec);
        rtc_alarm.enabled = 1;
        rtc_set_alarm(alarm_rtc_dev, &rtc_alarm);
    }else{
        pr_alarm( ERROR, "alarm_shutdown, clear rtc alarm type %d\n",ANDROID_ALARM_POWEROFF_WAKEUP);

        rtc_alarm.enabled = 0;
        rtc_time_to_tm(0, &rtc_alarm.time);
        rtc_set_alarm(alarm_rtc_dev, &rtc_alarm);
        tps80031_clear_alarm();
    }
}
EXPORT_SYMBOL_GPL(alarm_shutdown);
#endif

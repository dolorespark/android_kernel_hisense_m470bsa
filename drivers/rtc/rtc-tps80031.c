...
extern struct i2c_client *tps80031_i2c_client;
    ...
    uint8_t *val)
{
    ...
    /* dummy read of STATUS_REG as per data sheet */
    ret = tps80031_reads(dev, 0x1, RTC_STATUS, 1, val);
    if (ret < 0) {
    ...
    ret = tps80031_reads(dev, sid, reg, len, val);
    if (ret < 0) {
...
static int tps80031_rtc_write_regs(struct device *dev, int sid, int reg, int len,
    uint8_t *val)
{
    int ret;
    ret = tps80031_writes(dev, sid, reg, len, val);
    if (ret < 0) {
        dev_err(dev, "\n %s failed writing\n", __func__);
        WARN_ON(1);
        return ret;
...
int tps80031_clear_alarm(void)
{
        ...
        err = tps80031_rtc_read_regs(dev, 0x1, RTC_INT, 1, &reg);
        if (err)
            return err;
        reg &= 0xF7;
        err = tps80031_rtc_write_regs(dev, 0x1, RTC_INT, 1, &reg);
        if (err)
            return err;
        if(err) {
            dev_err(dev, "\n can't clear alarm irq\n");
            return err;
		}
        ...
        buff[5] = tm.tm_year % RTC_YEAR_OFFSET;
        convert_decimal_to_bcd(buff, sizeof(buff));
        err = tps80031_rtc_write_regs(dev, 0x1, RTC_ALARM, sizeof(buff), buff);
        if (err)
            dev_err(dev, "\n unable to clear alarm\n");
        return err;
}
EXPORT_SYMBOL_GPL(tps80031_clear_alarm);
#endif

...
#define TPS80031_CFG_INPUT_PUPD1_PREQ2_PU (0x1<<3)
#define TPS80031_CFG_INPUT_PUPD1_PREQ2_PD (0x1<<2)
#define TPS80031_CFG_INPUT_PUPD1_PREQ1_PU (0x1<<1)
#define TPS80031_CFG_INPUT_PUPD1_PREQ1_PD (0x1<<0)
...
#define TPS80031_ID3_TRIM_EXTEND		0xFD ... 0xFF
...
struct i2c_client *tps80031_i2c_client;
#ifdef CONFIG_PM
static int tps80031_set_preq1_pullup(int enable);
#endif
...
#ifdef CONFIG_BATTERY_GAUGE_TPS8003X
	{
...
static void tps80031_power_off(void)
{
	...
	if (!tps->client)
        ...
        extern void alarm_shutdown(void);
        alarm_shutdown();
#endif
...
//FG can only be closed in the case of having standalone fg or using adc_bat driver instead of gauge driver.
static bool can_gauge_close_suspend(void)
{
#ifdef CONFIG_BATTERY_BQ27x00
...
int tps80031_close_gpadc(void)
{
    ...
    tps80031_read(dev, 2, reg, &val);
    printk("++GADAC: reg[0x%x]:0x%x\n", reg, val);

    //ret = tps80031_clr_bits(dev, 2, reg, mask);
    val=0x01;
    ret = tps80031_update(dev, 2, reg,val, 0x03);
    tps80031_read(dev, 2, reg, &val);
    printk("--GADAC: reg[0x%x]:0x%x\n", reg, val);
    if(can_gauge_close_suspend()) {
    //close FG
    tps80031_read(dev, 2, reg, &val);
    printk("++GADAC: reg[0x%x]:0x%x\n", reg, val);

    val=0x10;
    ret = tps80031_update(dev, 2, reg,val, 0x30);
    tps80031_read(dev, 2, reg, &val);
    printk("--GADAC: reg[0x%x]:0x%x\n", reg, val);
    }
    else
    printk("--GADAC: Keep FG driver\n");
				...
				int irq_base)
{
	...
	if (!ret)
	{
	        //printk("%s(): register the isr for the secondary interrupt, irq = %d\n", __func__, irq_base + TPS80031_INT_CHRG_CTRL);
					...
					const struct i2c_device_id *id)
{
	...
	tps80031_i2c_client = client;
        ...
        //added for debug vdd_core request
        //reg:0x48-->0xF0
        //value: 0x56 (default 0x55)
        //internal plug-up the PREQ1
    tps80031_set_preq1_pullup(0x1);
//end
    //printk("========= default value ========\n");
    //tps80031_print_regs();    
...
static int tps80031_set_preq1_pullup(int enable)
{
    uint8_t value = 0;
    int ret = 0;
    ret = tps80031_read(tps80031_dev->dev, SLAVE_ID1, TPS80031_CFG_INPUT_PUPD1, &value);
    if (ret >= 0)
	    printk("1   SLAVE_ID1->0xF0->PREQ1 Value 0x%02x\n", value);
    if(enable)
        ret = tps80031_update(tps80031_dev->dev, SLAVE_ID1, TPS80031_CFG_INPUT_PUPD1,\
                TPS80031_CFG_INPUT_PUPD1_PREQ1_PU,0x03);
    else
        ret = tps80031_update(tps80031_dev->dev, SLAVE_ID1, TPS80031_CFG_INPUT_PUPD1,\
                TPS80031_CFG_INPUT_PUPD1_PREQ1_PD,0x03);
    
    if (ret < 0){        
        printk("**********Not able to write reg\n");
        return -1;
    }

    ret = tps80031_read(tps80031_dev->dev, SLAVE_ID1, TPS80031_CFG_INPUT_PUPD1, &value);
    if (ret >= 0)
	   printk("2   SLAVE_ID1->0xF0->PREQ1 Value 0x%02x\n", value);
    return 0;
}

#if 0
static void tps80031_print_int_sys_regs(struct tps80031 *tps80031)
{
    int ret = 0;
    u32 acks;
    uint8_t tmp[3];
...
static int tps80031_i2c_suspend(struct device *dev)
{
	...
	struct tps80031 *tps80031 = i2c_get_clientdata(client);
        ...
        //printk("suspend: ");
        //tps80031_print_int_sys_regs(tps80031);
	
	if (client->irq)
		disable_irq(client->irq);
        tps80031_close_gpadc();

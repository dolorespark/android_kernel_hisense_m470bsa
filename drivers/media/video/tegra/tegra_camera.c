...
{
	int err;
	unsigned long value;//huang add
	struct tegra_camera_dev *dev;
printk("tegra_camera_probe comein huangyongheng add .\n");
	dev_info(&pdev->dev, "%s\n", __func__);
...
//huangyongheng add start
	value = clk_get_rate(dev->vi_clk);
	err = clk_set_rate(dev->vi_clk, 24*1000*1000);
 
////clk_enable(dev->vi_clk);
	value = clk_get_rate(dev->vi_clk);

	value = clk_get_rate(dev->vi_sensor_clk);
	printk("clk_set_rate is success\n");
	//huangyongheng add end
...
{
printk("tegra_camera_init comein huangyongheng .\n");

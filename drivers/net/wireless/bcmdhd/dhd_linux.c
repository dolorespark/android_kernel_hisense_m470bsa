...
/* Test mode flag, liuqiang add */
char firmware_test_path[] = "/etc/broadcom/wifi/fw_bcmdhd_test.bin";
uint dhd_test_mode_flag = 0;
module_param(dhd_test_mode_flag, uint, 0644);
...
dhd_open(struct net_device *net)
{
	...
	//liuqiang add for test mode
	if (dhd_test_mode_flag) {
		strcpy(fw_path, firmware_test_path);
	}else{
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (!ret) {
		...
		DHD_ERROR(("%s: get default MAC address , %.2x:%.2x:%.2x:%.2x:%.2x:%.2x\n", __FUNCTION__,
	...
	{
  		int roam_trigger = -85;
  		dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_TRIGGER, (char*)&roam_trigger, sizeof(roam_trigger), TRUE, 0);
  		DHD_ERROR(("roam_trigger = -85\n"));
	}
